...
def normalized_bbox(bboxes, image_w, image_h):
    ...
    normalized_bbox = np.concatenate((scaled_x, scaled_y, scaled_x + scaled_width, scaled_y + scaled_height, scaled_width, scaled_height), axis=1)
    return normalized_bbox


def compute_on_dataset(model, data_loader, device, bbox_aug, timer=None, output_folder=None):
    model.eval()
    results_dict = {}
    ...
    for _, batch in enumerate(tqdm(data_loader)):
        ...
        image_h, image_w = images.tensors.shape[2:]

        # rpn_bboxes = output[0].bbox.numpy()
        # rpn_features = output[0].get_field('box_features').numpy()
        # rpn_soft_labels = output[0].get_field('scores_all').numpy()
        # rpn_conf = output[0].get_field('scores').numpy()
        # rpn_cls = output[0].get_field('labels').numpy()

        det_bboxes = output[0].get_field('bbox_det').numpy()
        # det_soft_labels = output[0].get_field('scores_all_det').numpy()
        det_conf = output[0].get_field('scores_det').numpy()
        det_cls = output[0].get_field('labels_det').numpy()
        det_feats = output[0].get_field('det_feats').numpy()
        # rpn_norm_bb = normalized_bbox(rpn_bboxes, image_w, image_h)
        det_norm_bb = normalized_bbox(det_bboxes, image_w, image_h)
        # image_ids = image_ids[0].replace('.jpg', '.npz')
        # outputfile = osp.join(output_folder, image_ids)

        # det_features = output[0].get_field('box_features_det').numpy()

        # np.savez_compressed(outputfile,
        #                     features=rpn_features.astype(np.float16),
        #                     norm_bb=rpn_norm_bb.astype(np.float16),
        #                     conf=rpn_conf.astype(np.float16),
        #                     cls=rpn_cls,
        #                     soft_labels=rpn_soft_labels.astype(np.float16),
        #                     det_norm_bb=det_norm_bb.astype(np.float16),
        #                     det_conf=det_conf.astype(np.float16),
        #                     det_cls=det_cls,
        #                     det_soft_label=det_soft_labels.astype(np.float16))
        
        np.savez_compressed(outputfile,
                            det_norm_bb=det_norm_bb,
                            det_conf=det_conf,
                            det_feats=det_feats,
                            det_cls=det_cls)
...
def inference(
    ...
    if output_folder and os.path.isfile(os.path.join(output_folder, output_pth_name)):
        logger.info("Predictions.pth file exist in {}, skip computation".format(os.path.join(output_folder, output_pth_name)))
        if not is_main_process():
            return
        ...
        if eval_attributes:
            ...
            predictions = compute_on_dataset(model, data_loader, device, bbox_aug, inference_timer, output_folder)
